...
func TestBossFloorEntry(t *testing.T) {
	...
	events := register + enter + killMonster1 + killMonster2 + nextFloor + enterBoss
	...
	if !contains(outputs, "Player [1] entered the boss's floor") {
		t.Errorf("Expected boss floor entry message not found")
	}
}

func TestBossFloorBoundary(t *testing.T) {
	config, err := loadConfig("test_config.json")
	if err != nil {
		t.Fatalf("Failed to load config: %v", err)
	}
	...
	register := "[14:00:00] 1 1\n"
	enter := "[14:00:01] 1 2\n"
	killBossOnFloor0 := "[14:00:02] 1 7\n"

	outputs, err := testProcessReader(strings.NewReader(register+enter+killBossOnFloor0), gameInstance.InputEventChan, gameInstance.OutputEventChan)
	if err != nil {
		t.Fatalf("Failed to process events: %v", err)
	}

	if !contains(outputs, "Player [1] makes imposible move [7]") {
