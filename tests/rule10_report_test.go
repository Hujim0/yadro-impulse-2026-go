...
func TestFinalReportFormat(t *testing.T) {
	...
	if player1.Id != 1 {
		t.Errorf("Expected player ID 1, got: %d", player1.Id)
	}
	if player1.State != gamestate.SUCCESS {
		t.Errorf("Expected SUCCESS state, got: %s", player1.State)
	}
	if player1.Hp < 0 || player1.Hp > 100 {
		t.Errorf("Expected HP between 0-100, got: %d", player1.Hp)
	}
}

func TestFinalReportEdgeCases(t *testing.T) {
	config, err := loadConfig("test_config.json")
	if err != nil {
		t.Fatalf("Failed to load config: %v", err)
	}
	...
	enterWithoutRegister := "[14:00:00] 1 2\n"

	outputs, err := testProcessReader(strings.NewReader(enterWithoutRegister), gameInstance.InputEventChan, gameInstance.OutputEventChan)
	if err != nil {
		t.Fatalf("Failed to process events: %v", err)
	}

	if !contains(outputs, "Player [1] is disqualified") {
